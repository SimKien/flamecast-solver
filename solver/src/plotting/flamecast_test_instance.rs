...
pub fn plot_flamecast_test_instance(instance: &FlamecastTestInstance, file_path: &str) {
    ...
    instance.sources_drains_embeddings.embeddings[0]
        .iter()
        .for_each(|embedding| {
            root.draw(&create_node(embedding, sources_color, sources_capacity))
                .unwrap();
        });
    ...
    instance.sources_drains_embeddings.embeddings
        ...
        .iter()
        .for_each(|embedding| {
            root.draw(&create_node(embedding, drains_color, drains_capacity))
                .unwrap();
        });
